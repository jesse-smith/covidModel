...
#'
#' `sirv0()` simulates a SIR model with vaccinations, a variant type with
...
plot_sirv0 <- function(x, .y = .data[["I"]] / 3, ylab = "Observed Cases", group = NULL) {
  gg_obj <- x %>%
    ggplot2::ggplot(
      ggplot2::aes(
        x = .data[["time"]],
